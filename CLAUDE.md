...
- **⏳ 사용자 진행분**: Google Ads 계정 개설(전문가 모드) → 머천트·GA4 연결 → 전환 `purchase` 가져오기 → **표준 쇼핑 캠페인**(Performance Max 아님 — 데이터 0에서 시작하는 단계라 상품별·검색어별로 보이는 쪽이 판단 가능) 일 10,000원·최대 CPC 800원 → **3주 유지**(3~4일 만에 만지면 학습 초기화). 네이버는 계정만 미리 개설.
