...
- **계정 설정 실측**: 자동 태그 켜짐(정상) · 통화 보고서 사용 중 · **⚠️ 자동 적용 = "디스플레이 네트워크 추가 기능 외 추천 5개 사용"**(구글이 승인 없이 설정 변경 — 꺼야 함) · 제외 키워드 없음.
...
- **⏭️ 남은 작업(우선순위)**: ①`금강다온홈페이지방문` 입찰 **타겟CPA → 클릭수 최대화 + 최대 CPC 1,000원**(전환 데이터가 엉망인데 타겟 CPA를 물려둔 게 광고가 안 나가는 주원인) ②**자동 적용 끄기** ③전환 액션 3개 생성 ④캠페인 전환 목표 지정 ⑤`공인중개사사무소방문` **일시중지**(사용자 결정: 방문 유도 빼고 홈페이지 유도만) ⑥예산 재배분.
- ⚠️ **Google Ads는 이 세션에서 조작 불가** — 브라우저 제어 수단이 없고, API는 개발자 토큰 심사(수일~수주)가 필요. GA4는 서비스계정에 **편집자 권한**을 주면 API로 처리 가능(현재는 읽기 전용).
