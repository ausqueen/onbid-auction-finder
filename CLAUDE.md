...
### ✅ hsrealty 네이버 스마트플레이스 등록·연결 (커밋 `18f0eab`)
- 08-09 신청분이 승인됨. **플레이스 ID `2010052055`** (`https://map.naver.com/p/entry/place/2010052055`).
- 사이트 3곳에 연결: `HS_NAVER_PLACE_URL` 상수 · **LocalBusiness JSON-LD 의 `sameAs`** · 푸터 위젯("네이버 지도: 현성리얼티 찾아오는 길") · 지역랜딩(page 303)의 지도 링크를 **주소검색 → 플레이스 상세로 교체**(주소검색은 위치만, 플레이스는 업체정보·전화·길찾기·리뷰가 함께 뜸). 되돌리기 백업은 옵션 `hs_widget_custom_html_bak_20260812`·`hs_page303_bak_20260812`.
- NAP 는 이미 일치(현성리얼티 / 031-520-5552 / 경기도 안산시 단원구 고잔로 55 지하1층 102-아28호).
- **상세설명·대표키워드 작성 지원**: 상세설명 초안 `/mnt/nas/temp/네이버플레이스_상세설명.txt`(1,511자). ⚠️ **네이버 상세설명은 허용 문자가 제한됨** — 한글·영문·한자·일본어·숫자와 `-_():&![],.%+~@*^'/?²℃※<>` 만 가능. **`■`·`·`(가운뎃점)은 거부됨** → 소제목은 `[대괄호]`, 가운뎃점은 쉼표로. 검증 스크립트 `place_desc.py`.
- 대표 키워드 5개(사용자 등록 완료): `시놀로지` `나스` `안산 나스` `시흥 나스` `나스 설치`. ※ `사무실 나스`·`기업용 나스`는 데이터랩 지수 0.006 으로 사실상 검색이 없어 제외.
- ⏭️ 남은 것: **사진 등록**(완성도 점수에 직접 반영). 지역 검색 반영은 며칠~2주 소요.

### ✅ realty99 업체명·로고 애셋 비승인 해결 (daon 커밋 `40cc591`, `1ae6307`)
- **원인**: Ads 에 등록한 업체명은 붙여쓴 `금강다온공인중개사사무소` 인데 **광고 랜딩(홈·/properties·/auction)에는 띄어쓴 표기만** 있었음(붙여쓴 형태는 `/about` 에만). 게다가 **realty99 에 구조화 데이터가 하나도 없어** 검색엔진이 업체 정보를 확인할 근거 자체가 없었음. 로고도 같은 사유 — **사이트에 로고 이미지가 아예 없었음**(Header 는 텍스트, public/images 는 인증서·히어로 사진뿐).
- **조치**: `frontend/src/components/seo/OrganizationJsonLd.tsx` 신설(`RealEstateAgent`), layout 에 상시 렌더.
  - `alternateName` 에 **3표기 선언**(`금강다온공인중개사사무소`·`금강다온부동산`·`금강 다온`) — 기존 광고 제목에도 "금강다온부동산"이 쓰임
  - address·geo(37.3864, 126.8028)·telephone·areaServed(장현동 등 6개 동)·employee(박선영·원유호)·identifier(사업자·개설등록·매수신청대리 번호)
  - **`logo`** = `https://realty99.co.kr/icon.png` (파비콘 512x512 "DD" 마크 — 사이트에 존재하는 유일한 브랜드 마크)
  - ⚠️ **영업시간은 일부러 넣지 않음** — 사이트에 표기가 없어, 확인 안 된 값을 선언하면 잘못된 정보가 검색결과에 노출됨.
- ⏭️ 사용자가 Ads 에서 업체명 재제출(인증 진행 중) · 로고를 `/icon.png` 로 교체 예정. 구글이 랜딩을 재크롤해야 판정이 바뀌므로 1~2일 소요. **또 반려되면 로고는 삭제 권고**("DD"와 "금강 다온"의 시각적 연결이 약함).

### 📊 08-12 애셋 실적 (광고그룹 1 기준, 최근 30일)
- **사이트링크 CTR 2.75~3.75%** 로 광고 본문 CTR(1.43%)보다 훨씬 높음 — 확장소재가 제 몫을 하고 있음. 클릭 19·11·11·9·6.
- 신규 RSA 2개(중개·대리입찰) **광고 효력 "매우 좋음"** 확정(전날 '보류중'이던 중개 그룹 포함). 기존 광고는 '좋음'이며 클릭 96 을 유지 중 — 2~3주 뒤 성과 비교 후 정리 판단.

### ✅ hsrealty 네이버 파워링크 캠페인 — 검색광고 API로 이 서버에서 직접 등록 (2026-08-12 저녁)
비즈채널 승인 직후, PC 브라우저 작업 대신 **네이버 검색광고 API**(api.searchad.naver.com)로 서버에서 전량 등록. 스크립트·검증본은 `/mnt/nas/temp/hsrealty_파워링크_20260812/`(`naver_powerlink_setup.py`·`verify_setup.py`, 멱등 — 캠페인·그룹은 이름으로 재사용, 키워드는 100개 배치+실패 시 개별 재시도).
- **API 자격증명**: `~/.secrets/naver_searchad.txt`(600, ACCESS/SECRET/CUSTOMER 콜론 구분). ⚠️ **CUSTOMER_ID = `4471033`** — 광고주센터 URL의 광고계정 번호(2572987)가 아니라 **SA API 화면에 표시되는 별도 번호**다. 2572987로 넣으면 403 auth-failed(첫 시도에서 실제로 겪음. 시계·서명·키 형식 다 정상인데 403이면 이것부터 의심).
- **등록 결과** (캠페인 `현성리얼티 파워링크` cmp-a001-01-000000010963415 재사용, **하루예산 5,000원 강제 설정** — 사용자가 만든 캠페인이라 예산 미설정 상태였음):
  - 광고그룹 3개 신설(입찰가 200원 균일): `브랜드일반`(키워드 50·소재 4·홈) / `모델명`(키워드 273·소재 3·홈) / `설치구축`(키워드 15·소재 3·`/setup-service/`)
  - **키워드 338개 전량 등록, 거부 0** — `+` 포함 모델명 36개(DS925neo+ 등)도 전부 통과
  - **소재 10개 전부 즉시 APPROVED**
  - 사용자가 수동 생성해둔 빈 광고그룹(`현성리얼티_광고그룹#1_브랜드일반`, 입찰 70원·키워드 0)은 **userLock 으로 일시중지**(노출 불가 상태였지만 정리 차원)
- **API 함정 3개**: ①비즈채널 필드는 `nccChannelId`가 아니라 **`nccBusinessChannelId`** ②광고그룹 생성에 **`adgroupType: "WEB_SITE"` 필수**(없으면 3734 "광고그룹 유형이 없습니다") ③캠페인 예산 변경은 `PUT /ncc/campaigns/{id}?fields=budget`.
- 비즈머니 10만원 충전돼 있음(일 5,000원 기준 20일치). **전화 비즈채널(0315205552)도 등록돼 있음** — 전화·위치 확장소재는 UI에서 추가 가능(API 스키마 불확실해 시도 안 함).
- ⏭️ **2~3주 뒤 실적 보고 클릭 발생 키워드만 입찰가 인상**(API로 가능 — 자격증명 서버 보관 중).
- **✅ 전환추적 공통키 교체(08-13, 커밋 `e95c7d9`)**: 설치안내메일의 공통 스크립트와 대조 → 사이트의 wa 가 애널리틱스 키(`24d8291b3271480`)였음 → **검색광고 공통키 `s_a2abbf61379`로 교체** + `_nasa` 초기화 추가. wcs.trans 전환 스크립트는 유지. ⚠️ 애널리틱스 키와 광고 공통키는 별개 발급이며 wa 에는 하나만 들어감 — 광고 측정 우선으로 교체(방문 분석은 GA4 주력). ⏭️ **사용자가 메일의 "데이터 검수요청" 클릭하면 끝**(담당자 031-520-5552, 전환유형 구매·신청·장바구니, 테스트 키워드 '시놀로지 나스').

### ✅ 금강다온 파워링크도 API 로 점검·재구성 (2026-08-12 저녁)
- **자격증명**: `~/.secrets/naver_searchad_daon.txt`(600). **금강다온 CUSTOMER_ID = `2981006`** (hsrealty 4471033 과 별개 — 라이선스는 광고계정 단위).
- **점검 결과**: 08-11 등록한 키워드 **635개 전량 승인(거부 0)** · 두 캠페인 모두 하루예산 10,000원 설정돼 있음 · 첫 실적(8/10~12) 파워링크 노출 2,396/클릭 3/**CTR 0.13%**(교체 전 0.03%의 4배)/평균순위 2.4, 노출 발생 키워드 127개(상위 = 신일해피트리·장곡동아파트·군자서희스타힐스 등 실제 매물 단지) · 플레이스 캠페인 CTR 0.74%/순위 1.9.
- **병목 발견 → 재구성**: 광고그룹 1개에 635키워드 + **소재 1개(RSA_AD)** — 중개·대리입찰이 같은 광고 하나를 쓰는 구조. → `daon_restructure.py`(NAS `파워링크키워드_20260811/`에 보존)로 **그룹 2개 분리**: `대리입찰`(147, →`/auction`) / `부동산중개`(488, →`/properties`), 그룹별 맞춤 소재 3개씩(전부 즉시 APPROVED), 기존 그룹은 userLock 일시중지(되돌리기 가능). 키워드 거부 0.
- ⚠️ **API 캠페인 ID 를 하드코딩하지 말 것** — 점검 스크립트의 폴백 조회로 얻은 ID 를 그대로 박았다가 404(1018). 캠페인은 `campaignTp=WEB_SITE` 로 동적 조회.
- **플레이스 캠페인도 API 점검(08-12)**: 하루예산 10,000원 설정됨 · **요일·시간·지역 타겟이 전부 삭제된 상태**(= 08-11 확대가 완전 적용, MEDIA=naver·PC/모바일 all 만 남음) · 소재 2개(LOCAL_AD) 승인. **확대 효과 실측**: 8/4~10 노출 평균 ~440/클릭 0~4 → **8/11 노출 821(+86%)/클릭 10/CTR 1.22%**, 8/12 노출 811/클릭 6. 비용은 일 500~750원 수준. 전화·길찾기 전환은 스마트플레이스 통계에서만 확인 가능(광고 API 밖).
- **입찰가 전체 최저(70원)로 인하(08-12 저녁, 사용자 결정)**: 실적상 클릭당 232원(VAT 포함) = **상한 200원에 붙어 과금**되고 있었음(아래 경쟁자가 밀착해 있다는 뜻). `대리입찰`·`부동산중개` 두 그룹 bidAmt 200→**70원**, 키워드 635개 전부 그룹 입찰 상속(개별입찰 0개) 확인. 일시중지된 구 그룹은 건너뜀. 트레이드오프 = 노출 순위 하락으로 노출·클릭 감소 가능 — 며칠 실적 보고 판단.
- **⚠️ 08-13 사고·수습: 재구성이 콘텐츠 지면 폭주를 유발** — 새 광고그룹 2개를 **매체 타겟 없이(기본값=전체 매체)** 만든 탓에, 생성 직후(08-12 20시)부터 **디시인사이드·에펨코리아·나무위키 등 네이버 제휴 콘텐츠 지면**에 광고가 깔림. `신천역부동산` 하루 노출 66,608회(검색으로는 불가능한 양)·클릭 36회 등 이틀간 176클릭 ₩17,635 소진(일 예산 1만원 거의 소진). **어뷰징 아님** — GA4 대조로 확정: 유입 시작이 정확히 그룹 생성 시각, 새벽 0~5시 연속 유입, 이탈률 ~100%·체류 0초, 리퍼러가 m.dcinside.com/m.fmkorea.com/namu.wiki. **수습**: 두 그룹 MEDIA_TARGET을 `{type:2, search:["naver"], contents:[]}`(네이버 검색만)로 변경 완료.
  - 📌 **교훈 1**: API로 광고그룹을 만들면 **매체 타겟 기본값이 '전체 매체'** — 반드시 생성 직후 MEDIA_TARGET을 명시할 것(UI 생성과 기본값이 다를 수 있음).
  - 📌 **교훈 2**: 타겟 수정 API는 target 객체에 **`black`/`white` 속성까지 요구**("black property is missing" 400). 전체 구조: `{type, search, contents, black:{media:[],mediaGroup:[]}, white:{media:null,mediaGroup:null}}`, `PUT /ncc/adgroups/{id}?fields=targetMedia`.
  - 📌 **교훈 3**: 검색 키워드에 물리적으로 불가능한 노출량(수만/일)이 보이면 콘텐츠 지면부터 의심. hsrealty 쪽 새 그룹 3개도 같은 기본값일 것 — 확인 필요.
- **08-13 매체 전략 확정(사용자 결정)**: **부동산(금강다온) = 검색 지면만**(지역·의도가 전부라 콘텐츠 지면 무의미) / **NAS(hsrealty) = 커뮤니티 지면 확대하되 격리**. hsrealty 본 캠페인 3그룹은 검색 전용으로 유지하고, **별도 캠페인 `현성리얼티 콘텐츠확산`**(cmp-...10967051, **일 2,000원**·전 매체·입찰 70원 균일) 신설 — 그룹 `콘텐츠`에 브랜드·용도 키워드 65개 + 브랜드일반 소재 4종 재사용. 커뮤니티 클릭이 본 캠페인 검색 예산(일 5,000원)을 잠식하지 않도록 지갑을 분리한 것. 2주 뒤 검색 캠페인과 전환율 비교해 유지/종료 판단. 총 일 예산 상한 = 본 5,000 + 콘텐츠 2,000 = 7,000원.
