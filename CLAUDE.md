...
### ⛔ GA4 전환 가져오기 — 3일째 실패, 구글 서버측 문제로 확정
- 이벤트 3개 선택·카테고리 매핑·개별 설정(가치 사용 안 함 / 전환 1회만 / 기본 액션)까지 전부 마쳤으나 저장에서 동일 오류.
- **저장 직전 구글이 본인 인증을 요구해 사용자가 인증까지 마쳤는데도 같은 오류** → 인증 문제가 아니라 **순수 서버측 저장 API 문제**로 확정. 반복 시도 중단.
- 다음에도 실패하면 오류 대화상자의 **"Google Ads 지원팀에 문의"** 경로를 쓸 것. 캠페인 전환 목표 지정은 가져오기 성공 후에만 가능해 함께 보류.

### ⏭️ 남은 것
- 중개 광고그룹 **광고 효력 재확인**(1~2일 뒤)
- 캠페인의 **업체명 애셋 "금강다온공인중개사사무소"가 '비승인(관련성 없는 업체 이름)'** 상태 — 재제출 검토
- GA4 전환 가져오기 재시도 또는 지원팀 문의
